...
class TestViews(TestCase):
    def setUp(self):
        ...
        term2 = TermDates.objects.create(name = 'test2', start_date='2023-01-01', end_date='2023-12-31')
        term2.save()
    ...
    def test_renew(self):
        response = self.client.get(reverse('renew_lesson', args=[1]))
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, 'student_lessons.html')
        self.assertEqual(Lesson.objects.count(), 2)
        
    def test_renew_reponse_again(self):
        response = self.client.get(reverse('renew_lesson', args=[1]))
        response = self.client.get(reverse('renew_lesson', args=[2]))
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, 'student_lessons.html')
        self.assertEqual(Lesson.objects.count(), 2)